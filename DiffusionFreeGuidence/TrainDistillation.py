...
def train(modelConfig: Dict):
    ...
    ckpt = torch.load(os.path.join(
        modelConfig["teacher_save_dir"], modelConfig["training_load_weight"]), map_location=device)
    teacher.load_state_dict(ckpt)
    print("Teacher load weight done.")
    teacher.eval()
    trainer = GaussianDiffusionDistillationTrainer(
        teacher, net_model, modelConfig["beta_1"], modelConfig["beta_T"], modelConfig["T"], 14).to(device)
