...
class DownSample(nn.Module):
    ...
    def forward(self, x, temb, cemb, wemb):
...
class ResBlock(nn.Module):
    ...
    def forward(self, x, temb, labels, w):
        h = self.block1(x)
        h += self.temb_proj(temb)[:, :, None, None]
        h += self.cond_proj(labels)[:, :, None, None]
        h += self.guidance_proj(w)[:, :, None, None]
        h = self.block2(h)
...
class DistillationUNet(nn.Module):
    def __init__(self, T, num_labels, W, ch, ch_mult, num_res_blocks, dropout):
        ...
        self.guidance_embedding = GuidanceEmbedding(W, ch, tdim)
    ...
    def forward(self, x, t, labels, w):
        # Timestep embedding
        temb = self.time_embedding(t)
        cemb = self.cond_embedding(labels)
        wemb = self.guidance_embedding(w)
        # Downsampling
        h = self.head(x)
        hs = [h]
        for layer in self.downblocks:
            h = layer(h, temb, cemb, wemb)
            hs.append(h)
        # Middle
...
if __name__ == '__main__':
    ...
    w = torch.randint(14, size=[batch_size])
